...
var login = function login(options) {
    ...
    var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
        ...
        //自定义头信息
        header[constants.WX_HEADER_TANENT] = "tat0003"
        ...
        wx.request({
            ...
            success: function (result) {
                ...
                // 成功地响应会话信息
                if (data && data.code === 0 && data.data.skey && data.data.token) {
                    var res = data.data
                    if (res.userinfo) {
                        Session.set({
                            skey: res.skey,
                            userInfo: res.userinfo,
                            "x-okapi-token": res.token,
                            "x-okapi-tenant": header[constants.WX_HEADER_TANENT]
                        });
                        //从session中得到userInfo,修改了原来逻辑 yangxz
