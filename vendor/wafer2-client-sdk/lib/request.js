...
var buildAuthHeader = function buildAuthHeader(session) {
    ...
    if (session) {
        ...
        header[constants.WX_HEADER_TANENT] = session["x-okapi-tenant"];
        header[constants.WX_HEADER_TOKEN] = session["x-okapi-token"];
    }
    ...
    function doRequest() {
        ...
        wx.request(utils.extend({}, options, {
            ...
            success: function (response) {
                ...
                var error, message;
                if ((data && data.code === -1) || data == "Invalid token signature") {
                    Session.clear();
